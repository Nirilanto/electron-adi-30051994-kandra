// src/modules/settings/components/MotifTypesSettings.js
import React, { useState } from 'react';
import { PencilSquareIcon, TrashIcon, PlusIcon } from '@heroicons/react/24/outline';

function MotifTypesSettings({ motifTypes, onSave, onDelete }) {
  const [editMode, setEditMode] = useState(false);
  const [newMotifType, setNewMotifType] = useState({ title: '', label: '' });
  const [editingId, setEditingId] = useState(null);
  
  // Initialiser le formulaire d'édition avec les données du motif sélectionné
  const handleEdit = (motif) => {
    setEditingId(motif.id);
    setNewMotifType({ ...motif });
    setEditMode(true);
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-800">Motifs</h2>
      ...
      {editMode && (
        <form onSubmit={handleSubmit} className="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
          <h3 className="text-lg font-medium text-gray-700 mb-4">
            {editingId ? 'Modifier le motif' : 'Ajouter un motif'}
          </h3>
      ...
      {motifTypes.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white border border-gray-200">
            ...
            <tbody>
              {motifTypes.map(motif => (
                <tr key={motif.id} className="hover:bg-gray-50">
                  <td className="py-3 px-4 border-b">{motif.id}</td>
                  <td className="py-3 px-4 border-b">{motif.title}</td>
                  <td className="py-3 px-4 border-b">{motif.label}</td>
        ...
        <div className="text-center py-4 bg-gray-50 border border-gray-200 rounded-lg">
          <p className="text-gray-500">Aucun motif configuré</p>
          {!editMode && (
            <button
              type="button"
              onClick={() => {
...
export default MotifTypesSettings;