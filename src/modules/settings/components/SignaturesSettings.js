...
import { PencilSquareIcon, TrashIcon, PlusIcon, PhotoIcon, InformationCircleIcon } from '@heroicons/react/24/outline';
...
function SignaturesSettings({ signatures, onSave, onDelete }) {
  ...
  // Séparer les signatures et les tampons
  const signaturesList = signatures.filter(sig => sig.type === 'signature');
  const stampsList = signatures.filter(sig => sig.type === 'stamp');
      ...
      {editMode && (
        <form onSubmit={handleSubmit} className="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
          <h3 className="text-lg font-medium text-gray-700 mb-4">
            {editingId ? 'Modifier la signature' : 'Ajouter une signature'}
          </h3>
        ...
        <div>
          ...
          {signaturesList.length > 0 ? (
              ...
              {signaturesList.map(signature => (
                <div key={signature.id} className="border rounded-md p-4 bg-white shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="font-medium text-gray-900">{signature.title}</h4>
            ...
            <div className="text-center py-8 bg-gray-50 border border-gray-200 rounded-lg">
              ...
              <p className="text-gray-500">Aucune signature configurée</p>
            </div>
          )}
        </div>
      </div>

      {/* Bouton d'ajout si aucun élément */}
      {signatures.length === 0 && !editMode && (
        <div className="text-center py-8">
          <button
            ...
            className="text-blue-600 hover:text-blue-800"
          >
            Ajouter votre première signature ou tampon
          </button>
        </div>
      )}
