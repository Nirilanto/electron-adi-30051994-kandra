...
class SettingsService {
  constructor() {
    ...
    this.signaturesKey = 'settings_signatures'; // Nouvelle clé pour les signatures et tampons
  }
  ...
  async getTaxRates() {
    ...
    if (!taxRates || taxRates.length === 0) {
      const defaultTaxRates = [
        { id: 1, title: "TVA NORMALE", value: 20.0 },
        { id: 2, title: "TVA REDUIT", value: 5.5 },
      ];
      ...
      const defaultMethods = [
        {
          id: 1,
          title: "VIRA30JLDE05",
          label: "VIREMENT 30 JOURS LE 5 DU MOIS",
          month: 0,
          day: 5,
        },
        {
          id: 2,
          title: "VIRFINMOISDE15",
          label: "VIREMENT FIN DE MOIS LE 15",
          month: 0,
          day: 15,
        },
  ...
  async getJustificatifTypes() {
    ...
    if (!justificatifs || justificatifs.length === 0) {
      const defaultJustificatifs = [
        {
          id: 1,
          title: "ACCROISSEMENT TEMP. D'ACTIVITE",
          label: "ACCROISSEMENT TEMP. D'ACTIVITE",
        },
        {
          id: 2,
          title: "REMPLACEMENT SALARIE ABSENT",
          label: "REMPLACEMENT SALARIE ABSENT",
        },
        { id: 3, title: "RENFORT DE PERSONNEL", label: "RENFORT DE PERSONNEL" },
      ];
    ...
    if (!measures || measures.length === 0) {
      const defaultMeasures = [
        { id: 1, title: "CASQUE", label: "PORT DU CASQUE OBLIGATOIRE" },
        {
          id: 2,
          title: "CHAUSSURES",
          label: "CHAUSSURES DE SÉCURITÉ OBLIGATOIRES",
        },
  ...
  async getSignatures() {
    const signatures = await this.db.get(this.signaturesKey);
    if (!signatures || signatures.length === 0) {
      const defaultSignatures = [];
      await this.db.set(this.signaturesKey, defaultSignatures);
  ...
  async saveSignature(signature) {
    ...
    if (signature.id) {
      ...
      const newId =
        signatures.length > 0
          ? Math.max(...signatures.map((s) => s.id), 0) + 1
          : 1;
      signatures.push({ ...signature, id: newId });
    }
    return this.db.set(this.signaturesKey, signatures);
  }

  async deleteSignature(id) {
    const signatures = await this.getSignatures();
    const filteredSignatures = signatures.filter((s) => s.id !== id);
    return this.db.set(this.signaturesKey, filteredSignatures);
  }
}
