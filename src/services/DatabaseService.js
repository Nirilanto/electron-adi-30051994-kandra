class DatabaseService {
    ...
    async initializeDatabase() {
      try {
        ...
        const isInitialized = localStorage.getItem(`${this.storePrefix}initialized`);
        ...
        if (!isInitialized) {
          ...
          localStorage.setItem(`${this.storePrefix}employees`, JSON.stringify([]));
          localStorage.setItem(`${this.storePrefix}clients`, JSON.stringify([]));
          localStorage.setItem(`${this.storePrefix}contracts`, JSON.stringify([]));
    ...
    async getContracts(filters = {}) {
      try {
        ...
        let contracts = await this.getAll('contracts', filters, 'start_date', 'DESC');
        ...
        for (let i = 0; i < contracts.length; i++) {
          ...
          try {
            ...
            console.log(`Impossible de récupérer les détails pour le contrat ${contract.id}:`, error);
          }
        }
    ...
    async getEndingSoonContracts() {
      try {
        ...
        console.error('Erreur lors de la récupération des contrats qui se terminent bientôt:', error);
        throw error;
      }
    }
        ...
        contracts.forEach(contract => {
          if (contract.status === 'active' && contract.billing_rate) {
            ...
            if (startDate >= firstDayOfMonth && startDate <= today) {
              ...
              revenueThisMonth += workDays * 7 * parseFloat(contract.billing_rate); // 7 heures par jour
            } 
            // Si le contrat était déjà actif avant ce mois-ci
            else if (startDate < firstDayOfMonth) {
              const daysSinceMonthStart = Math.ceil((today - firstDayOfMonth) / (1000 * 60 * 60 * 24));
              const workDays = Math.min(daysSinceMonthStart, 22);
              revenueThisMonth += workDays * 7 * parseFloat(contract.billing_rate);
            }
          }
        });
    ...
    async exportData() {
      try {
        ...
        const employees = JSON.parse(localStorage.getItem(`${this.storePrefix}employees`) || '[]');
        const clients = JSON.parse(localStorage.getItem(`${this.storePrefix}clients`) || '[]');
        const contracts = JSON.parse(localStorage.getItem(`${this.storePrefix}contracts`) || '[]');
        ...
        const data = {
          ...
          contracts,
          metadata: {
            lastId_employees: parseInt(localStorage.getItem(`${this.storePrefix}lastId_employees`) || '0'),
            lastId_clients: parseInt(localStorage.getItem(`${this.storePrefix}lastId_clients`) || '0'),
            lastId_contracts: parseInt(localStorage.getItem(`${this.storePrefix}lastId_contracts`) || '0'),
    ...
    async importData(data) {
      try {
        ...
        if (!data.employees || !data.clients || !data.contracts || !data.metadata) {
          throw new Error('Les données importées sont incorrectes ou incomplètes');
        }
        
        // Importer les données
        localStorage.setItem(`${this.storePrefix}employees`, JSON.stringify(data.employees));
        localStorage.setItem(`${this.storePrefix}clients`, JSON.stringify(data.clients));
        localStorage.setItem(`${this.storePrefix}contracts`, JSON.stringify(data.contracts));
        
        // Importer les métadonnées
        localStorage.setItem(`${this.storePrefix}lastId_employees`, data.metadata.lastId_employees.toString());
        localStorage.setItem(`${this.storePrefix}lastId_clients`, data.metadata.lastId_clients.toString());
        localStorage.setItem(`${this.storePrefix}lastId_contracts`, data.metadata.lastId_contracts.toString());
    ...
    async saveToFile(filename) {
      try {
        ...
        // En mode développement, nous utilisons l'API du processus principal
        if (window.electron) {
          return await window.electron.data.exportData(data, filename);
        } else {
          // En mode navigateur (pour test), on propose le téléchargement
    ...
    async loadFromFile(file) {
      try {
        ...
        // En mode développement, nous utilisons l'API du processus principal
        if (window.electron && typeof file === 'string') {
          data = await window.electron.data.importData(file);
        } else {
          // En mode navigateur (pour test), on lit le fichier
