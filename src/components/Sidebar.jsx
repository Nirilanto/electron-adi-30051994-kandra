    ...
    const checkTimeTrackingAlerts = async () => {
      try {
        const TimeTrackingService = (
          await import("../modules/timetracking/TimeTrackingService")
        ).default;
        ...
        console.error(
          "Erreur lors de la vérification des alertes de pointage:",
          error
        );
      }
    };
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3 min-w-0 flex-1">
                <p className="text-sm font-medium text-blue-900">
                  {timeTrackingAlert} pointage{timeTrackingAlert > 1 ? "s" : ""}{" "}
                  à valider
                </p>
                <p className="text-xs text-blue-800">Cliquez pour gérer →</p>
              </div>
            </div>
          </Link>
        </div>
      ...
      {timeTrackingAlert > 0 && isActive("/timetracking") && (
        <div className="flex-shrink-0 p-3 border-t border-blue-700">
          <div className="bg-green-400 text-blue-900 text-xs font-medium px-3 py-2 rounded-lg text-center shadow-md">
            ✨ {timeTrackingAlert} validation{timeTrackingAlert > 1 ? "s" : ""}{" "}
            en attente
          </div>
        </div>
      )}
